<template>
  <!-- Header -->
  <div class="tw:flex tw:items-center tw:gap-3 tw:px-4 tw:py-3 tw:border-b tw:border-gray-100 tw:flex-shrink-0 tw:bg-white">
    <button
      @click="$emit('back')"
    ...
    <!-- Avatar -->
    <div class="tw:w-9 tw:h-9 tw:rounded-full tw:bg-gradient-to-br tw:from-blue-400 tw:to-blue-600 tw:flex tw:items-center tw:justify-center tw:text-white tw:font-semibold tw:text-sm tw:flex-shrink-0 tw:overflow-hidden">
      <img v-if="selectedUser.avatar" :src="selectedUser.avatar" :alt="selectedUser.name" class="tw:w-full tw:h-full tw:object-cover" />
  ...
  >
    ...
    <!-- Empty state -->
    <div v-else-if="messages.length === 0" class="tw:flex tw:flex-col tw:items-center tw:justify-center tw:h-full tw:gap-3 tw:text-center tw:text-gray-400">
  ...
  <div
    ...
    class="tw:bg-red-50 tw:border-t tw:border-red-200 tw:px-4 tw:py-2 tw:flex-shrink-0"
  >
  ...
  <!-- Footer: input -->
  <div class="tw:flex tw:items-end tw:gap-2 tw:px-4 tw:py-3 tw:border-t tw:border-gray-100 tw:bg-white tw:flex-shrink-0">
    <input
      v-model="inputText"
      type="text"
...
async function markMessagesAsRead(snapshot: ReturnType<typeof onSnapshot> extends (a: any, b: infer R, c: any) => any ? never : any) {
  // Narrow: only mark docs where receiver == me and status != 'read'
  try {
    const batch   = writeBatch(firestore)
    let   hasWork = false

    // We re-read snapshot here via the messages ref after it's set
