<template>
  <div class="tw:px-4 tw:py-4">
    ...
    <div v-if="community.length > 0" :class="['tw:flex tw:flex-wrap tw:gap-3', isExpanded ? 'tw:max-h-96 tw:overflow-y-auto' : '']">
    ...
    <!-- No Community Members Message -->
    <div v-else class="tw:text-sm tw:text-gray-500 tw:py-4">
      No community members attending this event yet.
    </div>
  </div>
</template>
...
// Display members based on expanded state
const displayedMembers = computed(() => {
  if (isExpanded.value || props.community.length <= 8) {
    return props.community
  }
  return props.community.slice(0, 8)
})
...
const remainingCount = computed(() => {
  ...
  return props.community.length - 8
})
